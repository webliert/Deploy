...
# pd_control 函数现在不再被使用，您可以保留它也可以删掉
...
def main():
    # Load configuration
    config = load_config("tienkung2_lite.yaml")
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        while viewer.is_running() and time.time() - start < config['simulation_duration']:
            ...
            # ==========================================
            # 核心修改部分：直接输出位置给 d.ctrl
            # ==========================================
            
            # 1. 腿部关节：直接将神经网络输出的目标位置赋值给 d.ctrl
            d.ctrl[:config['num_actions']] = target_dof_pos
            
            # 2. 手臂关节（如果存在）：保持在 0 位置
            if n_joints > config['num_actions']:
                ...
                if d.ctrl.shape[0] > config['num_actions']:
                    d.ctrl[config['num_actions']:] = arm_target_positions
